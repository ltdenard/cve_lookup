...
class CVELookup:
    def __init__(self, api_key=None, output_file=None, sleep_time=2, reinitialize=False, base_dir=None):
        ...
        self.cve_output_file = os.path.join(self.BASE_DIR, "all_cves.json")
        if output_file:
            self.cve_output_file = output_file
    ...
    def save_to_file(self, data):
        Path(self.cve_output_file).parent.mkdir(parents=True, exist_ok=True)
        with open(self.cve_output_file, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4, ensure_ascii=True, sort_keys=True)
        print(f"Saved {len(data)} CVEs to {self.cve_output_file}")

    def main_runner(self):
        utc_now = datetime.datetime.now(datetime.timezone.utc)
        hours_24_ago = utc_now - datetime.timedelta(hours=24)
        hours_48_ago = utc_now - datetime.timedelta(hours=48)
        if not os.path.exists(self.cve_output_file) or self.reinitialize:
        ...
        if os.path.exists(self.cve_output_file) and not self.reinitialize:
            with open(self.cve_output_file, "r") as f:
        ...
        if (self.cve_json and (self.last_update_time < hours_24_ago)) or self.reinitialize:
            self.save_to_file(self.cve_json)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="all_cves.json", help="Output file (default: all_cves.json).")
    parser.add_argument(
        '--reinit',
        action='store_true',
    ...
    obj = CVELookup(output_file=args.output, reinitialize=args.reinit, base_dir=base_dir)
    obj.main_runner()